...
- `sidepanel.js` — renders tabbed, searchable item cards from `GET_ITEMS`, dispatches `RUN_ACTION` requests, and listens for `ITEMS_UPDATED` pushes. Cards with a detected contact email get a "Draft Email" action that calls the backend's `POST /generate-email` directly and opens a pre-filled Gmail compose tab via `chrome.tabs.create()`. Every card also exposes the backend's generic `status`/`notes`/`resurface_at` fields via `PATCH /posts/{id}`: opportunity items keep the "Mark as Applied" toggle (now backend-persisted, setting `status = "applied"`), other items get a plain editable status field, and every item gets an expandable notes editor and a "Snooze" date picker that sets `resurface_at`. Items whose `resurface_at` is due (`<= now`) sort to the top and get a "Resurfaced" pill. Falls back to sample data when run outside an extension runtime (e.g. previewing the HTML directly).
...
- `metrics.js` — local funnel metrics engine backed by `chrome.storage.local` (no backend round trip). Tracks lifecycle counters `captures_total`, `forms_opened`, `emails_drafted`. `captures_total` is incremented by the background service worker on every successful capture (see `extension/background/background.js`) so it stays a monotonic event count independent of the live post list; the other two are incremented by the sidepanel as the user opens an apply form or drafts an email. `applications_submitted` is *not* tracked here -- it's fetched from the backend's `GET /stats/applied-count` (an all-time count, independent of pagination) and kept in sync with an optimistic +/-1 in `sidepanel.js`'s `toggleApplied()`, so it survives `chrome.storage.local` being cleared and stays in sync across devices. The panel's stats header shows Total Captures and Conversion Rate (`applications_submitted / captures_total`).

### `backend/` — AI Backend
FastAPI service that turns raw captured text into structured, actionable data.
- `main.py` — `POST /capture` (extract + persist), `GET /posts`, `GET /posts/{id}`, `PATCH /posts/{id}` (partial update of `status`/`notes`/`resurface_at`), `GET /stats/applied-count` (all-time count of applied opportunities, independent of `GET /posts`'s pagination -- the sidepanel's conversion-rate numerator), `POST /generate-email`, `GET /health`. CORS is restricted to `chrome-extension://` origins by regex, since `/capture` and `/generate-email` are unauthenticated and trigger paid LLM calls.
